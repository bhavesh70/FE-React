...
const Cards = (props) => {
    const {name, avgRating, cuisines, costForTwo, locality, sla, cloudinaryImageId} = props.cards.info;
    return ( <div className="body">
    <div className="flex flex-col ml-[10px] gap-5 w-[350px] p-[7px] mb-2.5  border-solid hover:shadow-xl hover:border rounded-xl">
      <img className="card-image w-[350px] h-[210px] rounded-lg" src={CARD_IMG_URL+cloudinaryImageId} />
  ...
  export const withDiscount = (Cards) => {
    return (props) => {
        ...
        return (
            <div>
                <label className="absolute mt-3 ml-3 bg-black text-white bg-opacity-35">{header + subHeader} </label>
                <Cards {...props}/>
            </div>
        )
    }
